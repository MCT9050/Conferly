...
import type { SidebarTab, AppView, Reaction } from '../types';
...
export default function MeetingControls({
  ...
  setView, meetingDuration, participantCount,
  ...
  useEffect(() => {
    if (!showReactions && !showMore) return;
    const handler = (e: MouseEvent) => {
      if (showReactions && reactionsRef.current && !reactionsRef.current.contains(e.target as Node)) setShowReactions(false);
  ...
  const handleLeave = () => { stopMedia(); stopListening(); setView('dashboard'); };
