...
test.describe('Fix 1 — Meet premium feature entitlement contract', () => {
  test('Meet premium capabilities are centralized and product-scoped to Meet', async () => {
    ...
    expect(meetEntitlements).not.toContain("'class_20'");
    expect(meetEntitlements).not.toContain("'class_30'");
  });
  ...
  test('AI server actions fail closed without Meet room context', async () => {
    ...
    expect(aiActions).not.toContain("'class_20'");
  });
